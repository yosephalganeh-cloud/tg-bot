...
def start_bot():
    ...
    try:
        ...
        stop_flag = True
        print(f"\n{Colors.WARNING}[!] Process forcefully interrupted by user (Ctrl+C). Stopping immediately...{Colors.ENDC}")
    except Exception as e:
        print(f"\n{Colors.FAIL}[!] An unexpected error occurred: {e}{Colors.ENDC}")

    print(f"\n{Colors.BOLD}{Colors.GREEN}[✓] Broadcast Task Ended. Sent {successful_sends}/{num_messages} messages.{Colors.ENDC}")
    input(f"\n{Colors.CYAN}Press Enter to return to Main Menu...{Colors.ENDC}")

def delete_single_message(token, chat_id, msg_id):
    """Deletes a single message via Telegram API."""
    global stop_flag
    if stop_flag:
        return False, token, chat_id, msg_id
    ...
    try:
        with urllib.request.urlopen(req, timeout=5):
            ...
            return True, token, chat_id, msg_id
    except urllib.error.HTTPError as e:
        if e.code == 429:
            time.sleep(1)
        ...
        print(f"{Colors.FAIL}[X] Error deleting message ID: {msg_id}{Colors.ENDC}")
        return False, token, chat_id, msg_id
...
def delete_messages():
    ...
    if not os.path.exists(HISTORY_FILE):
        print(f"{Colors.WARNING}[!] No message history found in {HISTORY_FILE}.{Colors.ENDC}")
        time.sleep(2)
        return
    ...
    for line in lines:
        ...
        if len(line_data) == 3:
            ...
            if saved_chat_id == chat_id:
                target_items.append((token, saved_chat_id, msg_id))
            else:
    ...
    if not target_items:
        print(f"{Colors.WARNING}[!] No messages found for Chat ID: {chat_id}{Colors.ENDC}")
        time.sleep(2)
        return

    print(f"\n{Colors.CYAN}[*] Found {len(target_items)} messages. Launching fast deletion threads... Press Ctrl+C to stop.{Colors.ENDC}\n")
    
    deleted_count = 0
    deleted_msg_ids = set()

    try:
        max_threads = min(20, len(target_items))
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
            futures = [executor.submit(delete_single_message, item[0], item[1], item[2]) for item in target_items]
            
            for future in concurrent.futures.as_completed(futures):
                if stop_flag:
                    break
                success, t, cid, mid = future.result()
                if success:
                    deleted_count += 1
                    deleted_msg_ids.add(mid)

    except KeyboardInterrupt:
        stop_flag = True
        print(f"\n{Colors.WARNING}[!] Deletion immediately stopped by user (Ctrl+C).{Colors.ENDC}")

    # Safely update history file
    final_lines = list(keep_lines)
    for item in target_items:
        t, cid, mid = item
        if mid not in deleted_msg_ids:
            final_lines.append(f"{t},{cid},{mid}\n")

    with open(HISTORY_FILE, 'w') as f:
        f.writelines(final_lines)

    print(f"\n{Colors.BOLD}{Colors.GREEN}[✓] Deletion Task Completed! Deleted {deleted_count}/{len(target_items)} messages.{Colors.ENDC}")
    input(f"\n{Colors.CYAN}Press Enter to return to Main Menu...{Colors.ENDC}")
